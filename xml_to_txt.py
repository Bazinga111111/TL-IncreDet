...
#类名
class_names=['Bus','Truck','SUV','Microbus','Sedan','Minivan']
#选择输入路径
#path = 'D:/database/629_train/train_images/'   
path = 'D:/work/thesis/light weight/dataset/car_2/test/labels/'
#转换一个xml文件为txt
def single_xml_to_txt(xml_file):
    ...
    with open(txt_file, 'w') as txt_file:
        for member in root.findall('object'):
	        #从xml获取图像的宽和高
            picture_width = int(root.find('size/width').text.strip('[]'))
            picture_height = int(root.find('size/width').text.strip('[]'))
